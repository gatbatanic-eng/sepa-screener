...
----------------------------------
    python run_daily_screen.py --market KR        # 코스피+코스닥 시총 상위 N
    ...
    python run_daily_screen.py --market KR --top-n 100  # 시총 상위 100종목만
...
def run_market(market: str, top_n: int, limit: int | None) -> None:
    logger.info("=== %s 기술적 신호 스크리닝 시작 ===", market)
    records = run(market, top_n=top_n, limit=limit)
...
def main() -> None:
    ...
    parser.add_argument("--top-n", type=int, default=cfg.KR_TOP_N_DEFAULT,
                         help="한국 시장 시가총액 상위 몇 종목 (미국은 S&P500 전체 고정)")
    ...
    for m in markets:
        run_market(m, top_n=args.top_n, limit=args.limit)
